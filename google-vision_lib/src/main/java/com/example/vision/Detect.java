...
public class Detect {
  ...
  public static List<TextItem> detectText(String filePath, PrintStream out) throws Exception, IOException {
    List<AnnotateImageRequest> requests = new ArrayList<>();
    List<TextItem> listItems = new ArrayList<>();
    ByteString imgBytes = ByteString.readFrom(new FileInputStream(filePath));
...
/*        for (EntityAnnotation annotation : res.getTextAnnotationsList()) {
        ...
        for(int i = 0 ; i < res.getTextAnnotationsList().size() ; i++) {
        	EntityAnnotation entityAnnotation = res.getTextAnnotationsList().get(i);
        	...
        	item.setText(entityAnnotation.getDescription());
        	item.setxMin(xMin);
        	item.setxMax(xMax);
        	item.setyMin(yMin);
        	item.setyMax(yMax);
  ...
  public static List<TextItem> detectTextGcs(String gcsPath, PrintStream out) throws Exception, IOException {
    List<AnnotateImageRequest> requests = new ArrayList<>();
    List<TextItem> listItems = new ArrayList<>();
    ImageSource imgSource = ImageSource.newBuilder().setGcsImageUri(gcsPath).build();
    Image img = Image.newBuilder().setSource(imgSource).build();
